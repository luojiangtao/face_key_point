...
# 每个关键点分别取 0.16*w 0.16*h , 0.18*w 0.18*h 两种padding，取关键点周围一部分区域用来训练
types = [(0, 'LE1', 0.16),
         (0, 'LE2', 0.18),
         (1, 'RE1', 0.16),
         (1, 'RE2', 0.18),
...
def generate(ftxt, mode, argument=False):
    ...
    for idx, name, padding in types:
        ...
        with h5py.File('/python/face_key_point/data_hdf5/train/2_%s/%s.h5'%(name, mode), 'w') as h5:
            h5['data'] = patches.astype(np.float32)
            h5['landmark'] = landmarks.astype(np.float32)
        with open('/python/face_key_point/data_hdf5/train/2_%s/%s.txt'%(name, mode), 'w') as fd:
            fd.write('/python/face_key_point/data_hdf5/train/2_%s/%s.h5'%(name, mode))


if __name__ == '__main__':
    np.random.seed(int(time.time()))
    # trainImageList.txt
    generate('/python/face_key_point/data_image/trainImageList.txt', 'train', argument=True)
    # testImageList.txt
    generate('/python/face_key_point/data_image/testImageList.txt', 'test')
    # Done
